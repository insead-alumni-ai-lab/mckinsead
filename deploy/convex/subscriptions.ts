...
export const useSession = mutation({
  ...
  handler: async (ctx) => {
    ...
    if (sub.status !== "active") return { allowed: false, reason: "Subscription not active" };
    ...
    return { allowed: true, remaining: sub.sessionsLimit - sub.sessionsUsed - 1 };
  },
});
