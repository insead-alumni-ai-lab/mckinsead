...
export function ScopingPanel({
  ...
  const savedGates: string[] = engagement.gatesApproved
    ? (() => {
        try {
          return JSON.parse(engagement.gatesApproved!);
        } catch {
          return [];
        }
      })()
  ...
  const hasScqa = Boolean(
    saved?.situation || saved?.complication || saved?.question || saved?.answer,
  );
  const [mode, setMode] = useState<"input" | "review">(
    hasScqa ? "review" : "input",
  );
  const [showManualEntry, setShowManualEntry] = useState(false);

  // ── Problem description ──
  const [problemDescription, setProblemDescription] = useState(
    saved?.problemDescription ?? "",
  );
  ...
  const [question, setQuestion] = useState(
    saved?.question ?? engagement.question ?? "",
  );
  ...
  const handleInferSCQA = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.situation) {
        ...
        setError(
          result.error ??
            "Failed to infer SCQA. Please try again or enter manually.",
        );
      }
    } catch (err) {
      setError(
      ...
      <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <TextareaWithMic
                ...
                placeholder={
                  'e.g. "Our B2B SaaS CRM company grew 20% YoY but a competitor launched AI at half our price. We are losing enterprise deals."'
                }
                rows={5}
                className="text-sm"
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button
                onClick={handleInferSCQA}
                disabled={!problemDescription.trim() || inferring}
                className="gap-2"
              >
                {inferring ? (
                  <>
                    <Loader2 className="size-4 animate-spin" /> Analyzing with
                    AI...
                  </>
                ) : (
                  <>
        ...
        {mode === "review" && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              AI inferred SCQA structure. Edit any field as needed.
            </p>
            <ScqaFields
              {...{ situation, complication, question, answer }}
              onSituationChange={setSituation}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Gate G1: Approve problem framing before proceeding
                </span>
              </div>
              <Button
...
function ScqaFields({
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      <div className="space-y-2">
        <Label className="text-sm font-semibold flex items-center gap-2">
          <span className="size-5 rounded-full bg-blue-500/10 text-blue-600 flex items-center justify-center text-[10px] font-bold">
            S
          </span>
          Situation
        </Label>
        <TextareaWithMic
          ...
          onBlur={onBlur}
          rows={3}
          onTranscription={t => onSituationChange(t)}
        />
      </div>
      <div className="space-y-2">
        <Label className="text-sm font-semibold flex items-center gap-2">
          <span className="size-5 rounded-full bg-orange-500/10 text-orange-600 flex items-center justify-center text-[10px] font-bold">
            C
          </span>
          Complication
        </Label>
        <TextareaWithMic
          value={complication}
          onChange={e => onComplicationChange(e.target.value)}
          onBlur={onBlur}
          rows={3}
          onTranscription={t => onComplicationChange(t)}
        />
      </div>
      <div className="space-y-2">
        <Label className="text-sm font-semibold flex items-center gap-2">
          <span className="size-5 rounded-full bg-violet-500/10 text-violet-600 flex items-center justify-center text-[10px] font-bold">
            Q
          </span>
          Question
        </Label>
        <TextareaWithMic
          ...
          onBlur={onBlur}
          rows={3}
          onTranscription={t => onQuestionChange(t)}
        />
      </div>
      <div className="space-y-2">
        <Label className="text-sm font-semibold flex items-center gap-2">
          <span className="size-5 rounded-full bg-green-500/10 text-green-600 flex items-center justify-center text-[10px] font-bold">
            A
          </span>
          Answer (Hypothesis)
        </Label>
        <TextareaWithMic
          ...
          onBlur={onBlur}
          rows={3}
          onTranscription={t => onAnswerChange(t)}
        />
      </div>
    </div>
  );
