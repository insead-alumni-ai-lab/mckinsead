import { Check, CheckCircle2, Download, Loader2, Minus } from "lucide-react";
...
export function ExportPanel({
  ...
  const completedFrameworks = frameworkDataList.filter(
    f => f.status === "done" && f.data !== "{}",
  );
  const scopingData = engagement.scopingData
    ? (() => {
        try {
          return JSON.parse(engagement.scopingData!);
        } catch {
          return null;
        }
      })()
    : null;
  const synthesisData = engagement.synthesisData
    ? (() => {
        try {
  ...
  const themeColors: Record<
    string,
    { bg: string; accent: string; text: string; lightBg: string }
  > = {
    navy: {
      bg: "#1a237e",
      accent: "#283593",
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const fwSections = completedFrameworks
        .map(fw => {
          const label = frameworkLabels[fw.framework] || fw.framework;
          let parsed: Record<string, unknown>;
          try {
            parsed = JSON.parse(fw.data);
          } catch {
            return "";
          }
...
<div style="text-align:center;color:#999;font-size:9px;margin-top:40px;padding-top:16px;border-top:1px solid #eee">Generated by mckinsead — Agentic Strategy Cockpit</div>
      ...
      if (format === "pptx") {
        ...
        const blob = new Blob([JSON.stringify(slideData, null, 2)], {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Export your strategy analysis as a professional report
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="bg-muted/50 rounded-lg p-4 text-sm space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-1.5">
              {synthesisData?.governingThought ? (
                <Check className="size-3 text-green-600" />
              ) : (
                <Minus className="size-3" />
              )}
          ...
          {[
            {
              id: "pdf",
              label: "PDF Report",
              desc: "Print-ready strategy report",
              icon: "fa-solid fa-file-pdf",
            },
            {
              id: "html",
              label: "HTML Report",
              desc: "Browser-based, shareable",
              icon: "fa-solid fa-globe",
            },
            {
              id: "pptx",
              label: "PowerPoint / JSON",
              desc: "Slide-ready data export",
        ...
        {completedFrameworks.length === 0 && (
          <p className="text-xs text-amber-600 text-center">
            ⚠️ No framework data generated yet. Go to the Diagnose stage and
            generate analyses first.
          </p>
        )}
      </CardContent>
    </Card>
